...
namespace Moq
{
	...
	public abstract class LookupOrFallbackDefaultValueProvider : DefaultValueProvider
	{
		...
		protected void Deregister(Type factoryKey)
		{
			...
			// NOTE: In order to be able to unregister the default handling for awaitable types,
			// we need a way (below) to know when to delegate to `AwaitableHandler`, and when not to.
		...
		protected internal sealed override object GetDefaultValue(Type type, Mock mock)
		{
			...
			{
				if (factory != null)  // This prevents delegation to `AwaitableHandler` for deregistered awaitables; see note above.
				{
					return factory.Invoke(type, mock);
				}
			}
			else if (AwaitableHandler.TryGet(type) is { } awaitableHandler)
			{
				var resultType = awaitableHandler.ResultType;
				var result = resultType != typeof(void) ? this.GetDefaultValue(resultType, mock) : default;
				return awaitableHandler.CreateCompleted(result);
			}
