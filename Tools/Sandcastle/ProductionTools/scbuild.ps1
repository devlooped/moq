...
#
#	OPTIONS
#		-BuildAssemblerConfig {file} -- user specified BuildAssembler config file.
#			Default: The config file specified by the doc model.
#
#		-BuildChm, -BuildHxs, -BuildWebsite -- select build target.
#
#		-Clean -- remove Sandcastle temporary directory.
#
...
#
#			scbuild -framework 2.0 -sources test.dll,comments.xml
#
#   PREREQUISITES
#       - Sandcastle
#       - .NET 2.0
...
function Run {
    ...
    if ($Clean -or $BuildChm -or $BuildHxs -or $BuildWebsite) {
        Clean
    }
...
function BuildWebsite {
    FatalError "The Website build is not yet implemented."
}
...
# dependencies. The data is used as is from MrefBuilder. No post processing occurs.
#
function GenerateDependencyReflectionData($assemblies, $outputDir) {
    ...
    if ($assemblies) {
        foreach ($pn in $assemblies) {
            GenerateTargetReflectionData $pn "$outputDir\$([IO.Path]::ChangeExtension($pn.Name, '.xml'))"
