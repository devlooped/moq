...
{
	public class TaskOfHandlerFixture
	{
		[Fact]
		public async Task CreateCompleted__returns_completed_task__with_correct_result()
		{
			...
			var handler = new TaskOfHandler(typeof(int));
			var task = (Task<int>)handler.CreateCompleted(expectedResult);
		...
		public async Task CreateFaulted__returns_faulted_task__that_throws_correct_exception()
		{
			...
			var handler = new TaskOfHandler(typeof(int));
			var task = (Task<int>)handler.CreateFaulted(expectedException);
